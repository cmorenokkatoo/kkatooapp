...
class Apps extends CI_Controller {
	...
	/**
	* Verifica si el usuario puede utilizar la aplicaci�n
	*/
	...
	*/
	private function _load_step_two($result)
	{
		...
		*/
		...
		/** Informaci�n para la librer�a de contenidos **/
		// DATOS DE LA LIBRER�A DE CONTENIDOS
	...
	private function _load_step_one($result, $datacsv = array(), $csvname = "")
	{
		...
		if($this->_login_in())
		{
			...
			if(empty($campaign))
			{
				...
				//Si un usuario esta conectado se le inicia la campa�a automaticamente
			...
			else
			{
				...
				else
				{
					...
					//Busca los contactos filtrando por la campa�a
				...
				//Consulta los usuarios asociados a la campa�a que alguna vez inici�
	...
	*/
	private function _get_contacts_filter_campaign($campaign, $wapp, $fields, $filter = 0, $group = 0, $id_field = 0, $ope = "", $criterio = "")
	{
		...
		//Si no se va a filtrar se muestran todos los contactos que no han sido asignados a una campa�a
		if($filter == 0)
		{
			if($tipo == 1){
		...
		else //Se realizan los filtros respectivos y se traen los contactos con los criterios y no asignados a la campa�a
		{
	...
	*/
	private function _get_contacts($wapp = 0 , $fields = array(), $filter = 0, $group = 0, $id_field = 0, $ope = "", $criterio = "")
	{
		...
		else //Si filtra se envian los datos a la funci�n para que se encargue de hacer el filtrado de los usuarios
		{
			if($id_field)
			{
	...
	*/
	private function _get_contacts_subscribe($wapp = 0 , $fields = array(), $filter = 0, $group = 0, $id_field = 0, $ope = "", $criterio = "")
	{
		...
		// PARTE SIN LA VUELTA DE SUSCRIPCI�N EL ELSE
		...
		else //Si filtra se envian los datos a la funci�n para que se encargue de hacer el filtrado de los usuarios
		{
			...
			if(!empty($ids_filter))
			{
				...
				if($id_field)
				{
					//Los importantes para la p�ginaci�n son estos.
	...
	*/
	public function add_audio_record()
	{
		...
		else
		{
			...
			if(empty($uri))
			{
				echo 'cod=0&messa=No esta permitido realizar esta acci�n';
			}
			else
			{
				$this->load->model('apps_model');
				...
				if(empty($result))
				{
					echo 'cod=0&messa=La url de la aplicaci�n no existe';
				}
				else
				{
					$campaign 	= $this->apps_model->get_campaign($this->session->userdata('user_id'),$result->id);
					if(empty($campaign))
					{
						echo 'cod=0&messa=La campa�a para este usuario no existe o no ha sido creada';
					}
					else
					{
	...
	*/
	public function get_contacts()
	{
		...
		else
		{
			...
			//EJECUTAMOS LA MEGA FUNCI�N PARA LOS CONTACTOS
		...
		/**
	* Funcion publica para listar los contactos de una campa�a por AJAX
	*/
	...
	*/
	...
	/*
	* Funci�n para agregar a todos los contactos del filtro actual a la lista de seleccionados
	*/
	function add_all_contacts_to_campaing(){
		...
		if ($this->form_validation->run() == FALSE)
		{
			$this->session->set_flashdata('error',$this->lang->line('ocurrio_un_error'));
			redirect('marketplace');
			die();
		}
		else
	...
	/**
	 * Funcion publica para agregar un usuario a una campa�a
	*/
	...
	/**
	* Funcion publica para eliminar un contacto de una campa�a
	*/
	...
	/**
	 * Funcion publica para agregar un audio a una campa�a
	*/
			...
			if(!empty($campaign))
			{
				...
				else
				{
					...
					$result = $this->apps_model->insert_text_speech_campaign(
																				...
																				0, //Oj� esto es un fake para mandar algo de text speech pero la verdad si se debe mandar el id!. ojo!.
	...
	*/
	...
	/*
		Funcion publica para agregar el nombre de la campa�a
	*/
	...
	/*
	//Upload de audio para el PRIMER mensaje de la campa�a no para el arbol de llamada
	*/
	public function upload_audio_campaign_ini()
	{
		...
		$this->form_validation->set_rules('id_wapp', 'Id de la aplicaci�n', 'required|xss_clean|numeric');
		if ($this->form_validation->run() == FALSE)
		{
			$this->session->set_flashdata('error',validation_errors());
			redirect('apps/'.$url.'/2');
	...
	/*
		Funcion publica para traer informaci�n de un audio
	*/
	...
	*/
	function save_intro_out($type="intro"){
		...
		if(empty($wapp))
		{
			echo 'cod=0&messa=Por favor refresque la p�gina nuevamente.';
			die();
		}
		...
		if ( ! $this->upload->do_upload('Filedata'))
		{
			 echo "cod=0&messa=No se pudo subir el audio, por favor refresca la p�gina e intenta nuevamente.";
		}
		else
		{
			...
			if($x['Length'] !== NULL || $x['Length'] > 10)
			{
				...
				if($inser_au){
					...
					if($saved){
						...
						echo "cod=0&messa=No se pudo grabar el archivo, por favor refresca la p�gina e intenta nuevamente.";
					}
				}else{
					try{
						unlink('./public/audios/intro_cierre/'.$audio1['file_name']);
				...
				echo "cod=0&messa=La grabaci�n no debe ser mayor a 10 segundos.";
			}
		}
	}
	...
	*/
	...
	/**
	* Agregar contenido para el �rbol
	*/
	...
	*/
	...
	/*
		Funcion publica obtener informaci�n sobre un nodo...
	*/
				...
				else
				{
					...
					else
					{
						...
						if(count($marcado) != 0)
						{
							...
							{
								echo 'cod=0&messa=No se ha seleccionado opci�n de marcado'.$this->input->post('arbol');
							}
						}
						else
						{
	...
	*/
	...
	/**
	* Funcion publica para a�adir hora de llamada a una campa�a y finalizarla.
	*/
	...
	/**
	* Convierte los datos que deber�an ser tipo de dato fecha al formato m/d/Y
	* @param $date posible fecha que se debe converir
	* @return $rDate la fecha a retornar ya convertida, si sucede un error retorna la misma que se ingres�
	*/
	...
	/**
	* Funcion privada para dejar lista la campa�a
	*/
	public function _ready_campaign($campaign,$fecha,$hour,$minu,$timezones,$publisher,$ready,$isBuzonChecked,$isSMSChecked)
	{
		...
		//Calcular el tiempo estimado de duraci�n del audio
			...
			foreach($contacts as $data)
			{
				...
				foreach($tempMedia as $key => $med):
					...
					if($tempMedia[$key]['Type'] == 1)//and $media[0]['Indice']=="i")
					{
						...
						if (preg_match_all('/\[(.*?)\]/s',  $tempMedia[$key]['message'], $matches)){
							if (isset($matches[1]))
							{
								for($j=0; $j<count($matches[1]); $j++){

									$nombreAudio = $matches[1][$j];
									$pathAudio = $this->apps_model->get_audio_path_by_name($nombreAudio, $this->session->userdata('user_id'));
									$tempMedia[$key]['message'] = str_replace("[".$matches[1][$j]."]", "[".$pathAudio->path."]", $tempMedia[$key]['message']);
...
// echo "<pre>";
// 			echo $batch[0]['json'];
// 		print_r(json_decode($batch[0]['json']));
			//die();
	...
	/*
	* Funci�n para estimar el precio de la campa�a, por la duraci�n de la llamada
	*/
		...
		if(!empty($contacts))
		{
			foreach($contacts as $con)
			{
				...
				if(!empty($price))
				{
					...
					// 	$duration = 60;}
					// 	elseif($duration > 61 AND $duration < 120 ){
					// 	$duration = 120;}
					// 	elseif($duration < 180 AND $duration > 121){
					// 	$duration = 180;}
					// 	elseif($duration < 240 AND $duration > 181){
					// 	$duration = 240;}
					// 	elseif($duration < 300 AND $duration > 241){
					// 	$duration = 300;}
					// 	elseif($duration < 360 AND $duration > 301){
					// 	$duration = 360;}
					// 	elseif($duration < 420 AND $duration > 361){
					// 	$duration = 420;}
					// 	elseif ($duration == 0){
					// 	$duration = 0;}
	...
	*/
	...
	/**
	* Funci�n AJAX para pedir un audio profesional a la creadora de la aplicaci�n
	*/
	function request_profesional_audio(){
		...
		if(!empty($creator) && !empty($user)){
			try
			{
				...
				//log_message('debug', "Error enviando correo de kr�ditos por suscripci�n");
