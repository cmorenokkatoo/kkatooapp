...
class Videocreator extends CI_Controller {
	...
	{
		$this->deny_videocreator();
		
		//POCHO CÓDIGO
		ini_set('display_errors', 'on');
		$this->lang->load('apps');
		$this->load->model('videocreator_model');
		...
		if($this->_login_in()){
			...
			if($id_app !== FALSE){
				...
				$wapp = $this->videocreator_model->get_last_not_aproved_app($this->session->userdata('user_id'));
				if($wapp){
					redirect('videocreator/'.$wapp->id);	
				}else{
	...
	*/
	function newapp($tipo=false){
		...
		$id_wapp = $this->videocreator_model->initialize_app($this->session->userdata('user_id'), $tipo);
		redirect('videocreator/'.$id_wapp);	
	}
	...
	*/
	function crear_aplicacion_uris(){
		$this->load->model('videocreator_model');
		$apps = $this->videocreator_model->get_applications_without_uris();
		foreach($apps as $app){
			$uri 			=	 	$this->_quitarAcentos($app->title);
			$uri 			=   sanitize_title_with_dashes($uri);
			$result 	=  	$this->videocreator_model->get_uri($uri, $app->id);
			if(empty($result)){
				$this->videocreator_model->update_uri($app->id, $uri);
			}
		}
	}
	...
	*/
	function _load_app_data($result){
		...
		$category 			= $this->videocreator_model->get_category();
		$data["audios"] 	= $this->videocreator_model->get_library_audios_by_app($result->id, $this->session->userdata('user_id'));
		$data["records"] 	= $this->videocreator_model->get_library_records_by_app($result->id, $this->session->userdata('user_id'));
		$data["texts"] 		= $this->videocreator_model->get_library_texts_by_app($result->id, $this->session->userdata('user_id'));
		$data["library"]	= $this->videocreator_model->get_library_content($result->id, $this->session->userdata('user_id'));
		$data["packages"]	= $this->videocreator_model->get_packages_app($result->id);
		
		$data["dynamic"] 	= $this->videocreator_model->get_dynamic_fields($result->id);
	...
	*/
	...
	function save_info_app(){
		...
		if ($this->form_validation->run() == FALSE){
			...
			$this->session->set_flashdata('error',validation_errors());
			redirect('videocreator/'.$this->input->post($this->_id_app));
		}
		else{
			...
			if($this->check_if_owner($id_wapp)){
				...
				*/
				...
				$uri 			=   sanitize_title_with_dashes($uri);
				$result 	=  	$this->videocreator_model->get_uri($uri, $id_wapp);
				//$result 	= 	$this->videocreator_model->get_title($this->input->post($this->_nombre_aplicacion), $id_wapp);
				if($result)
				{
					...
					$updated	=	$this->videocreator_model->update_app_data($id_wapp, $data);
					if(!empty($updated) && ($data['private']==1 || $data['private']==0)){
						$this->videocreator_model->update_permission_app_by_private($data['private'], $id_wapp);
					}
					if(!empty($updated))
					{
						if($this->input->post($this->_dynamic_name))
						{
							if($this->input->post($this->_dynamic_type))
							{
								...
								if(count($name) < 10 and count($name) == count($type))
								{
									...
									for($i = 0; $i < count($name); $i++)
									{
										if($name[$i] != "" and isset($name[$i]))
										{
											...
											if(!empty($dinamic_id[$i])){
												$field_one = $this->videocreator_model->check_if_field_exits($dinamic_id[$i], $id_wapp);
												if($field_one){
													array_push($update, 
														array(
																			'id'					=> $dinamic_id[$i],
									...
									if(!empty($fields))
									{
										$batch 	= 	$this->videocreator_model->insert_batch_fields($fields);
									}
									if(!empty($update))
									{	
										//echo var_dump($update);
										$batch 	= 	$this->videocreator_model->update_batch_fields($update);
									}
								}
							}
						}
						...
						$this->videocreator_model->send_email_aprove($this->input->post($this->_nombre_aplicacion), $uri);
						redirect('videocreator/'.$id_wapp);
					}
				}
			}else{
	...
	function delete_dynamic(){
		...
		if ($this->form_validation->run() == FALSE){
			...
			if($this->check_if_owner($this->input->post('app_id'))){
				$success = $this->videocreator_model->delete_dynamic_field($this->input->post('dinamic_id'), $this->input->post('app_id'));
				if($success){
	...
	function upload_img(){
		...
		if(!empty($id_app)&&!empty($type)){
			...
			if ( ! $this->upload->do_upload($type)){
				...
				$current_img = $this->videocreator_model->return_imagen($id_app, $type);
				...
				$this->videocreator_model->update_app_img($id_app, 'img/apps/'.$imagen['file_name'], $type);
		...
		if ($this->form_validation->run() == FALSE){
			...
			{	
				...
				if($x['Length'] !== NULL)
				{
					...
					$inser_au 	= 	$this->videocreator_model->save_audio($insert, $this->input->post('id_wapp'), 'audio');
					
					if($inser_au){
						$audio_data = $this->videocreator_model->get_library_audio_by_id($inser_au);
						echo json_encode(array(
							'cod'			=> 1,
		...
		*/
		...
		else
		{
			...
			if($x['Length'] !== NULL)
			{
				...
				$inser_au 	= 	$this->videocreator_model->save_audio($insert, $id_wapp, 'audio');
	...
	*/
	function ajax_get_audio_recorded_data(){
		...
		$audio_data = $this->videocreator_model->get_library_audio_by_id($this->input->post('id_content'));
	...
	*/
	function ajax_delete_batch(){
		...
		if(is_array($valores) and !empty($valores)){
			...
			if(!empty($ids_text)){	
				$result_text_speech = $this->videocreator_model->batch_delete_text_speach(
			...
			if(!empty($ids_audio)){	
			$result_audio = $this->videocreator_model->batch_delete_audio(
																	$this->session->userdata('user_id'),
																	$ids_audio
																);
			}
	...
	*/
	...
	function ajax_save_text_speach(){
		...
		if ($this->form_validation->run() == FALSE){
			...
			if(!empty($posible_id)){
				$result = $this->videocreator_model->update_text_speach($data, $posible_id, $this->session->userdata('user_id'));
				if($result){
					$result = $posible_id;
				}else{
					echo json_encode(array(
				...
				$result = $this->videocreator_model->save_text_speech($data, $this->input->post($this->_id_app));
			}
			
			if(!empty($result)){
				//$data['id'] = $result;
				$data_return = $this->videocreator_model->get_library_texts_by_id($result);
				echo json_encode(array(
						'cod' => 1,
						'messa'	=> $data_return
				));
	...
	*/
	...
	function ajax_intro_cierre_update(){
		...
		if ($this->form_validation->run() == FALSE){
			...
			$result = $this->videocreator_model->intro_cierre_update($this->input->post('resp'), $this->input->post('id_wapp'), $this->input->post('action'), $this->session->userdata('user_id'));
			if($result){
	...
	*/
	function ajax_delete_text_speech(){
		...
		if ($this->form_validation->run() == FALSE){
			...
			$result = $this->videocreator_model->delete_text_speach($this->input->post('id_content'), $this->session->userdata('user_id'));
			if($result){
				echo json_encode(array(
									'cod' 		=> 1,
	...
	*/
	function ajax_delete_audio(){
		...
		if ($this->form_validation->run() == FALSE){
			...
			$result = $this->videocreator_model->delete_audio($this->input->post('id_content'), $this->session->userdata('user_id'));
			if($result){
				echo json_encode(array(
									'cod' 		=> 1,
									'messa'		=> $this->input->post('id_content')
	...
	*/
	function ajax_update_audio_name(){
		...
		if ($this->form_validation->run() == FALSE){
			...
			$result = $this->videocreator_model->update_audio_name($data, $this->input->post('id_content'), $this->session->userdata('user_id'));
	...
	*/
	...
	public function add_wapp()
	{
		...
		else
		{
			...
			else
			{
				...
				$result 	= 	$this->videocreator_model->get_uri($uri);
				if(!empty($result))
				{
				...
				else
				{
					...
					$id_wapp	=	$this->videocreator_model->insert_wapp($data);
					if(!empty($id_wapp))
					{
						$this->load->model('apps_model');
						...
						{	$audio1		=	$this->upload->data();
					        ...
					        if($x['Length'] !== NULL)
					        {
						        ...
						        $audio_app 						= 	$this->videocreator_model->insert_audio_wapp($aux);   
					        } 
					    }
					    //Subir audio Dos
	...
	/*
	Funcion privada cargar la vista de videocreator
	*/
	private function _view_videocreator($data = array())
	{
		$this->load->view('videocreator', $data);
	}
	...
	*/
	...
	/**
	* Verifica si tiene permisos para entrar al videocreator
	*/
	function deny_videocreator(){
		if($this->permissions->get('deny_videocreator')){
			$this->session->set_flashdata('error',$this->lang->line('nopermissionvideocreator'));
	...
	*/
	...
	function ajax_add_package(){
		...
		if($this->videocreator_model->get_package_by_app_and_value($id_wapp, $this->input->post('nro_package'))){
		...
		$result = $this->videocreator_model->add_package_application($data);
	...
	*/
	function ajax_remove_package(){
		...
		//$data = array('id_app'=>$id_wapp, 'amount'=>$this->input->post('nro_package'));
		$result = $this->videocreator_model->remove_packages($id_wapp,$this->input->post('id_package'));
