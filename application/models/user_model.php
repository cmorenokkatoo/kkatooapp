	...
	*/
	public function password_reset_user($email,$fullname,$token)
	{
		try
		{
			...
			$this->load->library('email');
			$this->email->from(KKATOO_EMAIL_INFO, 'Kkatoo Info');
		//	$this->email->from('juan.cuervo@domoti-sas.com', 'Kkatoo Info');
			$this->email->to($email);
			$this->email->subject($this->lang->line('subjectwelcome'));
			$this->email->message($mensaje);
...
// obtiene los créditos del usuario
	...
	/**
	* Obtener los usuarios registrados en la DB
	* @param $status enviar 0 para inactivos, 1 para acivos, y -1 para todos los usuarios, valor por defecto -1
	*/
	public function get_users_by_status($status = -1)
	{
		try
		{
			$this->db->select('*');
			...
			log_message('debug','Error al tratar de consultar a un usuarios por estado');
			return FALSE;
		}
	}
