...
class Queues_Model extends CI_Model {
	...
	*/
	...
	public function campaign_ready()
	{
		...
		{	
			...
			log_message('debug','Error al traer todas las campa�as listas para encolar menor a la fecha actual');
			return FALSE;
		}
	}
	...
	/** Obtiene los creditos de un usuario asociado a una aplicaci�n **/
	...
	function get_user_log_credits($id_user = null){
		try
		{
			$this->db->select('u.fullname, log.user_id,log.credits_before, log.credits_after');
			if(isset($id_user)) $this->db->where('log.user_id', $id_user);
			$this->db->join('user u', 'u.id=log.user_id_admin');
			

			
			$result = $this->db->get('log_credits log')->result();
			...
			log_message('debug','Error al tratar de consultar los logs de creditos del usuario '.$id_user);
			return FALSE;
		}
	}
	...
	/**
	*	Cambia el estado de una cola env�ada
	*/ 
	...
	/**
	* ACTUALIZA EL CR�DITO DEL USUARIO
	*/
	...
	/**
	* SUMA CR�DITO AL USUARIO GANADO EN LAS APLICACI�NES y REALIZA EL LOG
	*/
	...
	/**
	* SUMA CR�DITO A KKATOO GANADO EN LAS APLICACI�NES y REALIZA EL LOG
	*/
	...
	/**
	* ACTUALIZA EL CR�DITO DEL CONTACTO
	*/
	...
	/*
	* Calcula el precio según el contacto
	*/
	...
	public function get_contact_user($id_campaign_contact = 0)
	{
		try
		{
			...
			log_message('debug','Error al tratar de obtener informaci�n de un usuario');
			return FALSE;
		}
	}
	...
	*/
	...
	/**
	* Optiene el id del usuario dueno de la aplicaci�n
	* @param $id_app id de la aplicaci�n
	* @return Id del usuario due�o de la aplicaci�n
	*/
	...
	public function get_sms_state($id)
	{
		try
		{
			$this->db->select('tipo_sms'); 
			$this->db->where('id',$id);
...
*************************************************************************************************************************************************/
	...
	/** Funci�n que inserta 1 llamada a la tabla queues en estado 0 - uso exclusivo de la api**/
	public function insert_queue_api($id_campaign, $id_contact_campaign, $fecha, $hora, $minuto, $voice, $state, $phone, $pais, $json, $user_id, $seg_estimado, $gmt, $price_real, $id_contact, $publisher, $id_wapp, $tipo_wapp){
			
			
			
			$data = array(
				'id_campaign' =>  $id_campaign,
				'id_contact_campaign' => $id_contact_campaign,
				'fecha' 	  =>  $fecha,
				'hora'        =>  $hora,
				'minuto' 	  =>  $minuto,
				'voice' 	  =>  $voice,
	...
	/** Funci�n que inserta 1 llamada a la tabla queues en estado 0 - uso exclusivo de la api**/
	public function insert_queue_api_advanced($id_campaign, $id_contact_campaign, $fecha, $hora, $minuto, $voice, $state, $phone, $pais, $json, $user_id, $seg_estimado, $gmt, $price_real, $id_contact, $publisher, $id_wapp, $tipo_wapp, $request_date, $enterprice =null, $position = null, $enterprice_user = null, $document = null, $city = null){
			...
			if($this->db->affected_rows() > 0)
			{
				// Se insertan parametros adicionales para filtro de informacion en la tabla queues_aditional_filters
				
				$id_queue= $this->db->insert_id();
				
				$data_advanced = array(
				'id_queue' =>  $id_queue,
				'city' => $city,
				'position' 	  =>  $position,
				'user'        =>  $enterprice_user,
				'document' 	  =>  $document,
				'enterprice' 	  =>  $enterprice,
				'date_request' 	  =>  $request_date,
				'enterprice_user' 	  =>  $enterprice_user
				);
				$this->db->insert('queues_aditional_filters', $data_advanced);
	...
	public function get_wapp_price_api($id = 0)
	{
	...
	public function get_textspeech_campaign($id = 0)

	{
		$this->db->select('text_speech');
	 	$this->db->where('id', $id);
