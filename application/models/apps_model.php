...
class Apps_Model extends CI_Model {
	...
	*/
	...
	/*
	* Funcion obtener los contactos ya asociados a una campa�a
	*/
	public function get_contact_campaign($campaign, $user_id, $paginate = false)
	{
		try
		{
			...
			log_message('debug','Error al tratar de traer los contactos que asoci� a una campa�a');
			return FALSE;
		}
	}
	/*
	* Funcion para iniciar una campa�a para un usuario en una app
	*/
	public function get_campaign($user_id, $wapp)
	{
		try
		{
			...
			log_message('debug','Error al tratar de listar informaci�n de la campaign de un usuario');
			return FALSE;
		}
	}
	/*
	...
	*/
	public function ini_campaign($data)
	{
		try
		{
			...
			log_message('debug','Error al tratar de iniciar la campa�a de usuario');
			return FALSE;
		}
	}
	/*
	...
	*/
	public function get_contacts_wapp_filter_group($user_id = 0 ,$group = 0)
	{
		try
		{
			...
			/*
	* Funcion para listar los contactos disponibles pero con los filtros elegidos por el usuario pero del grupo solamente
	* para aplicaci�n de subscripci�n
	*/
	...
	*/
	...
	/*
	* Funcion para listar los contactos disponibles pero que no tienen campos de la aplicaci�n
	*/
	public function get_contacts_no_wapp($user_id = 0, $state ,$ids = array())
	{
		try
		{
			...
			log_message('debug','Error al tratar de listar los contactos de un usuario que no tienen campos de la aplicaci�n');
			return FALSE;
		}
	}
	...
	*/
	...
	/*
	* Funcion para listar los contactos disponibles pero que no tienen campos de la aplicaci�n por suscripci�n
	*/
	...
	/*
	* Funcion para listar los contactos disponibles de un usuario para una determinada campa�aa
	*/
	public function get_contacts_filter_campaign_wapp($user_id=0, $campaign=0,$wapp =0,$ids = array())
	{
		...
		try
		{
			...
			log_message('debug','Error al tratar de listar los contactos no asociados a una campa�a determinada');
			return FALSE;
		}
	}

	/*
	* Funcion para listar los contactos disponibles de una aplicaci�n para una determinada campa�aa
	*/
	public function get_contacts_filter_campaign_wapp_subscribe($campaign=0,$wapp =0,$ids = array())
	{
		...
		/*
	* Funcion para listar los contactos disponibles de un usuario para una determinada campa�a pero con filtros
	*/
	...
	public function get_mega_filtro_contacts($user_id = 0, $id_campaign = 0, $id_wapp = 0, $id_group = 0, $id_field = 0, $ope = "", $criterio = "", $dopage = true){
		...
		try{
			...
			//REVISO QUE TIPO DE APLICACI�N ES PARA PODER CREAR EL SELECT
			if($tipo == 1){
			...
			//FUERA LOS QUE YA EST�N EN LA CAMPA�A
			if(!empty($id_campaign)){
				$this->db->where('contacts_user.id NOT IN (select id_contact_user from contacts_campaign
											where id_campaign = '.$id_campaign.')', NULL, FALSE);
			}

			//SE DISTRIBUYE EL WHERE STATEMENT DEPENDIENTO DE QUE TIPO DE APLICACI�N ES.
			if($tipo == 1){
			...
			//REALIZAMOS EL FILTRO POR GRUPOS DEPENDIENDO EL TIPO DE APLICACI�N
			if(!empty($id_group) && $tipo != 1)
			{
			...
			if(!empty($id_field)){
				if(!empty($criterio)){
					...
					if($id_field == 999999 && $tipo == 1){
						...
						{
							//AC� NOS DAMOS CUENTA QUE EL FILTRO ES UN CAMPO DIN�MICO
							$this->db->where('contact_fields.id_fields', $id_field);
	...
	/*
	Funci�n para filtrar un array de contactos con los ya seleccionados en la campa�a
	*/
	...
	public function get_contacts_filter_campaign_wapp_filter($user_id, $campaign, $wapp, $group = 0, $id_field = 0, $ope = "", $criterio = "")
	{
		try
		{
			...
			log_message('debug','Error al tratar de listar los contactos no asociados a una campa�a determinada pero con filtros');
			return FALSE;
		}
	}
	...
	*/
	...
	/*
	* Funcion para obtener si una campaa�a tiene usuarios asociados
	*/
	public function get_campaign_contacts($campaign)
	{
		try
		{
			...
			log_message('debug','Error al tratar de averiguar si una campa�a tenia usuarios asociados');
			return FALSE;
		}
	}
	/*
	* Funcion para obtener informaci�n de un contacto en especifico de un usuario para saber si tiene la informaci�n necesaria
	*/
	...
	/*
	* Funcion para comprobar si un contacto ya fue asignado a una campa�a
	*/
	public function get_contact_campaign_user($contact, $campaign)
	{
		try
		{
			...
			log_message('debug','Error al tratar de obtener si un usuario esta ya en una campa�a');
			return FALSE;
		}
	}
	/*
	* Funcion para ingresar un contacto a una campa�a
	*/
	public function insert_contact_campaign($data)
	{
		try
		{
			...
			log_message('debug','Error al tratar de ingresar un contacto a una campa�a especifica');
			return FALSE;
		}
	}
		/*
	* Funcion para eliminar un contacto de una campa�a
	*/
	public function delete_contact_campaign($campaign, $contact)//, $user_id)
	{
		try
		{
			...
			log_message('debug','Error al tratar de eliminar un contacto de una campa�a');
			return FALSE;
		}
	}
	/*
	...
	*/
	...
	public function get_audio_app($user_id=0, $id = 0, $pos = 0, $limit=3)
	{
		try
		{
	...
	/*
	* Funcion para traer todos el audio de una campa�a
	*/
	public function get_audio_campaign($user_id, $campaign)
	{
		try
		{
			...
			log_message('debug','Error al tratar de obtener todos los audios de una campa�a');
			return FALSE;
		}
	}
	/*
	* Funcion para traer el texto de una campa�a para text_speech
	*/
	public function get_text_campaign($user_id,$campaign)
	{
		try
		{
			...
			log_message('debug','Error al tratar de obtener todos los textos de una campa�a');
			return FALSE;
		}
	}
	/*
	* Funcion para agregar audio a una campa�a de un usuario
	*/
	public function insert_audio_campaign($user_id  = 0, $id_campaign = 0,$audio = 0, $mensaje)
	{
		try
		{
			...
			log_message('debug','Error al tratar de agregar un audio a la campa�a');
			return FALSE;
		}
	}
	public function insert_auxiliar_campaign($user_id  = 0, $id_campaign = 0, $audio = 0, $mensaje)
	{
		try
		{
			$data = array(
	...
	*/
	public function get_audio_text_campaign($user_id, $id_campaign)
	{
		try
		{
			...
			log_message('debug','Error al tratar de saber si un usuario ya tiene audio o texto de una campa�a');
			return FALSE;
		}
	}
	/*
	...
	*/
	public function get_date_campaign($user_id, $id_campaign)
	{
		...
		/*
	* Funcion para agregar una fecha a una campa�a
	*/
	public function insert_date_campaign($user_id, $campaign, $fecha, $hora, $minu, $gmt, $tiposms)
	{
		try
		{
			...
			log_message('debug','Error al tratar de agregar fecha a una campa�a');
			return FALSE;
		}
	}
			/*
	...
	public function get_user_verified($user_id = 0, $verified = 0)
	{
		try
		{
			...
			return FALSE;
				...
				/*
	* Funcion para saber si una campa�a esta lista para despachar
	*/
	public function campaign_ready($user_id, $campaign)
	{
		try
		{
			...
			log_message('debug','Error al verificar una campa�a esta ready');
			return FALSE;
		}
	}
	/*
	* Funcion para traer toda la informaci�n de un audio
	*/
	public function get_audio($id)
	{
		try
		{
			...
			log_message('debug','Error al traer toda la informaci�n sobre un audio');
			return FALSE;
		}
	}
	/*
	...
	**************************/
	/*
		Funcion para obtener los campos de una aplicaci�n
	*/
	public function get_fields($wapp)
	{
		try
		{
			...
			log_message('debug','Error al tratar de obtener los campos de una aplicaci�n');
			return FALSE;
		}
	}
	/*
		Funcion para obtener el id de la aplicaci�n de una campa�a
	*/
	public function get_id_wapp($wapp)
	{
		try
		{
			...
			log_message('debug','Error al tratar de obtener el id de aplicacion de una campa�a');
			return FALSE;
		}
	}
	/*
	...
	*/
	...
	/*
	* Funcion para agregar el nombre a una campa�a
	*/
	public function add_name_campaign($id, $name)
	{
		try
		{
			...
			log_message('debug','Error al tratar de agregar un nombre a la campa�a');
			return FALSE;
		}
	}
	/*
	...
	*/
	public function update_marcado_campaign($user_id  = 0, $id_campaign = 0, $marcado)
	{
		...
		/*
	* Funcion para optener el precio por nro de tel�fono de un contacto.
	*/
	...
	*/
	...
	public function get_wapp_price($id_campaign = 0)
	{
		try
		{
			...
			log_message('debug','Error al tratar de obtener informaci�n de una app');
			return FALSE;
		}
	}
	...
	//POCHO CODIGO
	/**
	* Trae la uri de una aplicci�n
	* @param $id_app es el id de la aplicaci�n.
	* @return String con la uri de la aplicaci�n
	*/
	public function get_app_uri($id_app){
		try
		{
			...
			log_message('debug','Error al tratar de traer la uri de una aplicaci�n => get_app_uri');
			return FALSE;
		}
	}

	/**
	* Trae el procentaje a ganarse de una aplicci�n
	* @param $id_app es el id de la aplicaci�n.
	...
	*/
	...
	/**
	* Trae los datos de paquetes permitidos para la compra en las aplicaciones de suscripci�n
	* @param $id_app es el id de la aplicaci�n que tiene los paquetes asociados.
	...
	*/
	public function load_packages_suscription_data($id_app){
		try
		{
			...
			log_message('debug','Error al tratar de traer los paquetes de una aplicaci�n => load_packages_suscription_data');
			return FALSE;
		}
	}

	/**
	* Optiene el nro del paquete por id de paquete en suscripci�n
	...
	*/
	...
	/*F�rmula para precios en el Landing*/
	/**
	...
	*/
	public function create_price_by_package_suscription($id_package, $id_app, $id_contact){
		try
		{
			...
			if($this->check_package_app($id_package, $id_app)){
				...
				$price_call_kkatoo = $this->contacts_model->get_price_by_contact($id_contact);//Comisi�n del c�digo de pa�s seg�n el indicativo
			...
			log_message('debug','Error al tratar de crear el precio por paquete en una aplicaci�n => create_price_by_package_suscription');
			return FALSE;
		}
	...
	/**
	* function verifica si el paquete esta asociado a una aplicaci�n
	* @param $id_package id del paquete
	* @param $id_app id de la aplicaci�n
	...
	*/
	...
	public function check_package_app($id_package, $id_app){
		try
		{
			...
			log_message('debug','Error al tratar de buscar la asociaci�n de un paquete con una aplicaci�n => check_package_app');
			return FALSE;
		}
	}
	/**
	* Optiene el tipo de aplicaci�n por id
	* @param $id_app id de la aplicaci�n
	* @return valor del tipo de aplicaci�n 0 = super genral, 1 = suscripci�n o 2 = por difusi�n.
	*/
	function get_app_type($id_app = 0){
		try
		{
			...
			log_message('debug','Error al tratar de traer el tipo de una aplicaci�n => get_app_type');
			return FALSE;
		}
	}
	...
	*/
	...
	/**
	* Optiene los datos de id de la aplicaci�n y tipo de la aplicaci�n con el id de la campana
	...
	*/
	...
	/**
	*	Verifica si la aplicaci�n es de la persona logueada
	*/
	function check_app_user($id_app='', $id_user=''){
		try
		{
			...
			log_message('debug','Error al tratar de verificar la aplicaci�n => check_app_user');
			return FALSE;
		}
	}

	/**
	* Trae la informaci�n de la aplicaci�n por id
	* @param $id_wapp id de la aplicaci�n
	*/
	...
	function get_app_data_by_id($id_wapp){
		try
		{
			...
			log_message('debug','Error al traer la informaci�n de una aplicaci�n => get_app_data_by_id');
			return FALSE;
		}
	}

	/**
	* Trae la informaci�n de la aplicaci�n por id de el usuario due�o
	...
	*/
	...
	/**
	* Obtener los permisos de una aplicaci�n por us id.
	...
	*/
	...
	/**
	* Crea los datos de una aplicaci�n en la cookie
	*/
	...
	/**
	* Trae los datos de el usuario creador de una aplicaci�n por el id de esta
	* @param $id_wapp id de la aplicaci�n
	*/
	function get_user_data_by_app($id_wapp=0){
		try
		{
			...
			log_message('debug','Error al tratar de traer los datos de un usuario por id de la apliaci�n => get_user_data_by_app');
			return FALSE;
		}
	}

	/**Verifica si un usuario esta asociado a una aplicaci�n con pines**/
	...
	function check_user_exits_pines($id_user = '', $id_wapp = ''){
		try
		{
			...
			log_message('debug','Error al tratar de consultar a un usuario asociado a una aplicaci�n por difusi�n con pines');
			return FALSE;
		}
	}

		/**Optiene los creditos de un usuario asociado a una aplicaci�n**/
	...
	* @param $id_campaign es el id de la campa�a
	* @param $id_wapp es el id de la aplicaci�n
	...
	*/
	function save_intro_close($id_campaign='', $id_wapp='', $id_user='', $data=array()){
		try
		{
			...
			log_message('debug','Error en la funci�n save_intro_close');
			return FALSE;
		}
	}
	...
	*/
	function update_intro_close_campaign($id_user = '', $id_campaign='', $id_intro_close='', $type='', $messaje=''){
		try{
			...
			log_message('debug','Error en la funci�n update_intro_close_campaign');
			return FALSE;
		}
	}
	...
	*/
	 ...
	 *
	 * Parameter:
	 *  	$appId - Int Id de la app  que se eliminar�.
	 *		$user_id - Int Id del usuario propietario de la app
	 * 	Return:
	 *	  TRUE - La app se borr� correctamente.
	 *    FALSE - Bool Cuando ocurri� un error.
	 *
	 */
	public function deleteApp($appId, $user_id)
	 ...
	 // Funci�n para obtener el path del audio
	 public function get_audio_path($name = '', $user_id = 0)
	 {
	 		$this->db->select('path');
	 		...
	 		return $result;
	 					...
	 					/* API */
	 // Funci�n para insertar una campa�a desde el API
	 ...
	 // Lo primero es reconocer que la app(wapp.id) tiene un due�o/owner (wapp.user_id)
	 // El wapp.user_id puede crear un audio(audio.id o text_speech.id) en su wapp.id, esto va a content_wapp.id (join con id_wapp = wapp.id)
	 // Se requiere que any user.id registrado (if private) o no(if public) va a usar la wapp.id del wapp.user_id(owner) tenga acceso a cualquier contet_wapp.id del wapp.id
	 // Si any user.id registrado o no en wapp.id crea un content_wapp.id solo lo podr� usar ese user.id que cre� el content_wapp.id para esa wapp.id

	 // ** Funci�n que me da el owner de la aplicaci�n
	 public function new_get_owner_wapp($id_wapp = 0)
	 {
	 ...
	 // ** obtener id's de los contenidos del owner de la app
	 public function new_get_id_content_wapp($id_wapp = 0)
	 {
	 	$this->db->select('id_content'); // id_content = audio.id y text_speech.id
	 	$this->db->where('id_wapp',$id_wapp);
	 	$result = $this->db->get('content_wapp')->row();
	 ...
	 //**    Obtener el path del audio por su nombre
	 public function get_audio_path_by_name($nameAu = '', $user_id = 0)
	 {
	 	$this->db->select('path');
	 	$this->db->where('name', $nameAu);
	 	$this->db->where('user_id', $user_id);
	 	$result = $this->db->get('audio')->row();
	 	return $result;
	 }
