...
class Videocreator_Model extends CI_Model {
	...
	*/
	...
	public function insert_wapp($data = array())
	{
		try
		{
			...
			log_message('debug','Error tratando de insertar información del usuario');
			return FALSE;
		}
	}
	...
	public function insert_audio_wapp($data = array())
	{
		try
		{
			...
			log_message('debug','Error tratando de insertar audio a una aplicación');
			return FALSE;
		}
	}
	...
	// POCHO CÓDIGOS
	/*
	* Inicia una aplicación que se va a crear
	...
	public function initialize_app($id_user, $tipo){
		try{
			...
			if($this->db->affected_rows()>0){
				...
				$permissions[] = array('id_permission'=>9, 'id_wapp'=>$wapp_ID, 'state'=>0); //Aplicación de pin
			...
			log_message('debug','Error tratando de iniciar una aplicación => initialize_app');
			return FALSE;
		}
	}
	
	/**
	* Optiene los audios asociados a una aplicación en la librería de contenido
	* @param $id_app el id de la aplicación
	...
	*/
	function get_library_audios_by_app($id_app='', $id_user=FALSE){
		...
		try{
			...
			log_message('debug','Error de traer la libreria audio de una aplicación => get_library_audios_by_app');
			return FALSE;
		}
	...
	/**
	* Optiene las grabaciones asociadas a una aplicación en la librería de contenido
	* @param $id_app el id de la aplicación
	*/
	...
	/**
	* Optiene los texto speechs asociados a una aplicación en la librería de contenido
	* @param $id_app el id de la aplicación
	*/
	...
	/**
	* Optener el contenido de la aplicación
	*/
	...
	function get_library_content($id_app='', $id_user = FALSE){
		try{
			...
			$this->db->from('content_wapp');
			// Flag = 1 es el contenido que es original de la app y está disponible para todos los usuarios.
			if($id_user){
	...
	/**
	* Optiene los campos dínamicos de una aplicación
	...
	*/
	...
	/**
	* Actualiza las imagenes subidas por id de aplicación
	* @param $id_app el id de la aplicación
	* @param $url_img es la ruta de la imagen
	* @param $type el típo de imagen que se esta subiendo.
	*/
	function return_imagen($id_app='',$type=''){
		try
		{
			...
			log_message('debug','Error al traer la imagen de una aplicación');
			return FALSE;
		}
	}
	
	/**
	* Actualiza la información de una aplicación ya inicializada
	...
	*/
	function update_app_data($id_app='', $data=''){
		try
		{
			...
			log_message('debug','Error al actualizar la info de una aplicación');
			return FALSE;
		}
	}
	...
	*/
	function update_permission_app_by_private($private='', $id_wapp=0){
		try
		{
			...
			log_message('debug','Error al actualizar la info de permisos de una aplicación');
			return FALSE;
		}
	}

	/**
	* Inserta un permiso para una aplicación
	* @param $id_wapp id de la palicación
	...
	*/
	function insert_permission($id_wapp, $id_permission, $state){
		try
		{
			...
			log_message('debug','Error al insertar la info de un permiso de una aplicación');
			return FALSE;
		}
	}
	...
	*/
	function check_if_permission_exist($id_wapp='', $id_permission=''){
		try
		{
			...
			log_message('debug','Error al actualizar la info de un permiso de una aplicación');
			return FALSE;
		}
	}

	/**
	* Actualiza un permiso de una apliación
	* @param $id_wapp id de la palicación
	* @param $id_permission id del permiso
	* @param $state valor de actualización del permiso
	*/
	...
	*/
	function update_app_img($id_app='', $url_img='', $type=''){
		try
		{
			...
			log_message('debug','Error al tratar de agregar una imagen a la aplicación');
			return FALSE;
		}
	}
	
	/**
	* Optiene la última aplicación no aprovada del usuario actualmente conectado
	*/
	function get_last_not_aproved_app($id_user){
		try{
			...
			log_message('debug','Error al tratar de traer la última aplicación no aprovada por usuario');
			return FALSE;
		}
	}
	...
	*/
	function check_if_field_exits($id_field='', $id_app=''){
		try{
			...
			log_message('debug','Error al tratar de traer los datos de un campo dinámico');
			return FALSE;
		}
	}
	...
	*/
	public function update_batch_fields($data = array())
	{
		try
		{
			...
			log_message('debug','Error tratando actualizar campos dinámicos');
			return FALSE;
		}
	}
	
	/**
	* Elimina el campo dinámico de una aplicación y todos los datos asociados a este
	*/
	...
	try
		{
			...
			log_message('debug','Error tratando de eliminar campos dinámicos');
			return FALSE;
		}
	}
	...
	*/
	function save_text_speech($data=array(), $id_wapp=''){
		try
		{
			...
			log_message('debug','Error tratando de insertar información del texto speach');
			return FALSE;
		}
	}
	...
	*/
	function update_text_speach($data=array(), $id_content='', $user_id=''){
		try
		{
			...
			log_message('debug','Error tratando de actualizar información del texto speach');
			return FALSE;
		}
	}
	...
	*/
	...
	/**
	* Guarda la infromación de un audio que se sube.
	*/
	function save_audio($data=array(), $id_wapp='', $tipo = ''){
		try
		{
			...
			log_message('debug','Error tratando de insertar información del audio');
			return FALSE;
		}
	}
	...
	*/
	...
	/**
	* Función para actualizar los datos de grabar intro o cierre
	*/
	function intro_cierre_update($resp=0, $id_wapp=0, $action='', $user_id=''){
		try
		{
			...
			log_message('debug','Error al tratar de actualizar el intro o cierre de una aplicación');
			return FALSE;
		}
	}
	
	/**
	* Agrega el paquete a una aplicación
	...
	*/
	...
	/**
	* Obtiene un paquete por aplicación y valor
	* @param $id_wapp son los datos a agregar
	* @param $package_value es el id de la aplicación
	*/
	...
	/**
	* Función para remover un paquete en una aplicacióón
	*/
	...
	/**
	* Función para obtener los paquetes de una aplicación
	*/
	function get_packages_app($id_wapp = ''){
		try{
			...
			log_message('debug','Error al traer los paquetes de una aplicación => get_packages_app');
			return FALSE;
		}
	}
	...
	*/
	...
	/**
	* Retorna el usuario por aplicación
	*/
	...
	*/
	function send_email_aprove($name='', $uri=''){
		...
		$this->email->subject(sprintf('La aplicación %s se actualizó.', ucfirst($name)));
