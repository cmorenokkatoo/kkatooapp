...
var template_agregado = '<tr><td><i class="material-icons">clear</i></td><td style="text-align: left !important;"><i class="material-icons" style="vertical-align: middle !important;">person</i>{name}</td><td>({indi_pais}) {phone}</td></tr>';

var template_contacto = '<tr><td><input data-step="6" data-intro="Si deseas seleccionar algunos de los contactos puedes hacer click en el checkbox" type="checkbox" name="contactos[]" class="contactsch_remove contactsch" value="{id}"></td><td style="text-align: left !important;"><i class="material-icons" style="vertical-align: middle !important;">person</i>{name}</td><td>({indi_pais}) {phone} </td> {credits}';
...
function saveCampaignName(_name){
	...
	//setTimeout(function(){
	$.post("/apps/add_name_campaign",{'name':_name,'id_campaign':id_campaign_var},function(data){
...
/*$('input[name="txt-campaign-name"]').blur(function(){
	...
	$(".contactsch").live("change",function(){
		...
		$.post("/apps/add_contact_campaign", { id_contact: $(this).val(),id_campaign:id_campaign_var },
			   function(data) {
			    var obj = jQuery.parseJSON(data);
			    if(obj.cod == 1){
			    	actual.remove();
	...
	$(".deselect").live("click",function(){
		var actual = $(this).parent().parent();
		$.post("/apps/delete_contact_campaign", { id_contact: $(this).data("id"),id_campaign:id_campaign_var },
			   function(data) {
			    var obj = jQuery.parseJSON(data);
			    if(obj.cod == 1){
...
/*** #FUNCIONES AJAX PARA ENVIO DE DATOS**/
	...
	$("#selectall").on("click",function(){
		if($(this).is(':checked')){
			...
			$.post("/apps/batch_contact_campaign",{'id_campaign':id_campaign_var,'valores':contacts_to,'page':window.pageS},function(data){
...
function get_contacts_campaign(redraw){
	...
	$.post("/apps/get_contacts_campaign",{'id_campaign':id_campaign_var, 'page': window.pageS},function(data){
		var obj_int = jQuery.parseJSON(data);
...
function get_contacts(redraw){
	...
	$.post("/apps/get_contacts",filtro_global,function(data){
		var obj_int = jQuery.parseJSON(data);
...
$('form[name="form-add-all"]').on('submit', function(event){
	var form = $(this);
	//var form = $('<form>', {'action': '/apps/', 'method': 'post', 'id':'form-add-all'});
...
function cambiarCiudad(id_p){
	$.post("/apps/get_city",{id_pais:id_p},function(data){
		var obj_int = jQuery.parseJSON(data);
		var opciones = "";
		if(obj_int){
...
function paso2(){
	...
	$('#cbo_datos').change(function(){
	    var tav    = $('#txt-msg-to-speech').val(),
	        strPos = $('#txt-msg-to-speech')[0].selectionStart;
    ...
    $(".guardarprogreso").click(function(){
    	...
    	$.post("/apps/add_audio_campaign", { id_campaign: $(".id_campaign").val(), audio_text: "0",
    ...
    $(".enviartextmini").click(function(){
    	...
    	$.post("/apps/add_tree_audio_text", { id_campaign: $(".id_campaign").val(), audio_text: "0",
    ...
    $(".audioselectedmini").live("change",function(){
    	 ...
    	 $.post("/apps/add_tree_audio_text", { id_campaign: $(".id_campaign").val(), audio_text: "1", id_audio: $(".audioselectedmini:checked").val(), arbol:get_dial_selected() })
		.done(function(data) {

		});
		
		$.post("/apps/get_audio",{id_audio: $(".audioselectedmini:checked").val(),arbol:get_dial_selected()},function(data){
			if(data.cod == 1){
    ...
    $(".audioselected").live("change",function(){
	    ...
	    $.post("/apps/add_audio_campaign", { id_campaign: $(".audioselected:checked").data('campaign'), audio_text: "1", id_audio: $(".audioselected:checked").val() })
		.done(function(data) {
		  //alert("Data Loaded: " + data);
		});
		
		$.post("/apps/get_audio",{id_audio: $(".audioselected:checked").val()},function(data){
  ...
  var selected_text 	= $(".selected_text");
	...
	$('.filetree span.folder').live("click",function() {
		if($(this).hasClass('clicked')){
			...
			//get_number_tree
			$.post("/apps/get_number_tree",{arbol: get_dial_selected(),id_campaign: $(".id_campaign").val()},function(data){
				//console.log(data);
				if(data.cod != 1){
					show_error(data);
					return false;
	...
	$('div.delete_selected a').live('click',function(){
		...
		if(confirm('¿Esta seguro que desea eliminar la opción '+ String(arbol).slice(-1)+' seleccionada?')){
			$.post('/apps/delete_content_from_tree_campaign', {'id_campaign': id_campaign, 'arbol': arbol}, function(data){
...
function cambiarpaginacion(este){
	...
	$.post("/apps/get_audios_all",{pos: este.data("num"),id_campaign: $(".id_campaign").val()},function(data){
			console.log(data);
			$(".tabladesonidos").html("");
...
function cambiaraudios(audionuevo){
	$("#pre-cont-au audio").attr("src","/public/audios/"+audionuevo);
...
$('#request_audio').on('click', function(){
	...
	$.post("/apps/request_profesional_audio",{id_user: $(this).data('iduser'), id_app: $(this).data('idapp')},function(data){
...
$(document).on('click', '.telefono',function(event){
	...
	if($('input[name="arbol"]').val()!=""){
		var data = $('form[name="admin_contents"]').attr('action', "/apps/add_content_to_tree_campaign");
	}else{
		var data = $('form[name="admin_contents"]').attr('action', "/apps/add_content_to_main_campaign");
	}
	can_submit = true;
	data.submit();
	event.preventDefault();
