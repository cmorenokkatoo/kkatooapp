...
$(document).on('click', '.packages a', function(e){
	...
	$.post(root+'/wizard/ajax_remove_package', {'id_package':id_package, 'id_wapp': $app_data_id}, function(data){
...
$("#package").on('blur', function(event){
	...
	if(nro_packages){
		if(!doing){
			doing = true;
			$.post(root+'/wizard/ajax_add_package', {'nro_package':nro_packages, 'id_wapp': $app_data_id}, function(data){
				var dat = $.parseJSON(data);
				if(dat.cod == 1){
...
function renderizar_dinamicos_para_textspeach(){
	...
	$('input[name="dynamic_name[]"]', CAMPOS_DYNAMICOS).each(function(){
		...
		if(val){
			...
			$.post(root+'/wizard/sanitize_text', {text: val}, function(data){				
				var dat = $.parseJSON(data);
				if(dat.cod==1){
...
CAMPOS_DYNAMICOS.on('click', 'a.delete_field_database', function(event){
	if(confirm($sure_delete_dynamic)){
		...
		$.post(root+'/wizard/delete_dynamic', {dinamic_id: _dynamic_id, app_id: _app_id}, function(data){
			var dat = $.parseJSON(data);
			if(dat.cod==1){
				remover_campo(_this);
			}else{
...
function add_dynamic_field(){
	...
	select_tipo.append('<option value="4">Multiples valores, una selección</option>');
	select_tipo.append('<option value="5">Multiples valores, varias selecciones</option>');
...
$(function () {
	'use strict';
	var url = root+'/wizard/upload_img';
	...
	.bind('fileuploadsubmit', function (e, data) {
		var _this 		= $(this);
		_this.parent().find('.red, .green, .loading').remove();
		_this.parent().append($('<img>', {src:root+'/assets/img/loading.gif', 'class':'loading', 'width':26}));
	});
});